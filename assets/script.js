    ...
    .then(function (response) {
      ...
      fetch(
        ...
        .then(function (response) {
          ...
          // add Current day title
          var currentTitle = $("#current-title");
          currentTitle.text("Today's Weather:");
          ...
          currentWindSpeed.text(
            "Wind Speed: " + response.current.wind_speed + " KPH"
          );
        });
    })
    .catch(function (err) {
...
var fiveDayForecastSection = function (cityName) {
  ...
  fetch(
    ...
    .then(function (response) {
      ...
      fetch(
        `https://api.openweathermap.org/data/2.5/onecall?lat=${cityLat}&lon=${cityLon}&exclude=minutely,hourly,alerts&units=metric&appid=${apiKey}`
      )
        // get response from one call api and turn it into objects
        .then(function (response) {
          return response.json();
        ...
        .then(function (response) {
          ...
          for (var i = 1; i <= 5; i++) {
            ...
            futureTemp.text("Temp: " + response.daily[i].temp.day + " \u00B0C");

            // add humidity to 5 day forecast
            var futureWind = $("#future-wind-" + i);
            futureWind.text("Wind: " + response.daily[i].wind_speed + " KPH");
